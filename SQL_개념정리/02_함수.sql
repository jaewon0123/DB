...
함수는 SELECT 절, WHERE절, ORDER BY절, GROUPT BY절, HAVING절에서 사용 가능

SELECT 절 : 찾는 기준이 아닌, 눈으로 표기되는 총 합, 계산 결과 등에 대한 것을 
            눈으로 봤을 때 정리해놓은 컬럼
            -> 정리해서 보여주자
...
-- ↑ B 가 처음부터 쭉 글자를 봤을 때 몇 번째에 위치해있는가? 처음 B가 나타나는 위치는?

-- 문자열에서 5번 부터 검색해서 처음 찾은 'B'의 위치 조회
SELECT 'AABAACAABBAA', INSTR('AABAACAABBAA','B',5)
FROM DUAL;
-- AABAA 5번째부터 B를 찾기 시작
-- 'AABAACAABBAA' B가 9번째 저리에서 B가 처음 나타났기 때문에 B는 12번째 자리에 위치한다 표시된 것
...
SELECT EMP_NAME,SUBSTR(EMAIL,1,INSTR(EMAIL,'@')-1) AS "이메일_아이디"
FROM EMPLOYEE
ORDER BY "이메일_아이디";

-- INSTR(EMAIL,'@') 이메일 주소에서 @ 위치를 찾음
-- 찾은 다음에 @ 가 표시되면 안되기 때문에 -1 설정을 해서 @ 제거
SELECT EMP_NAME,SUBSTR(EMAIL,1,INSTR(EMAIL,'@')) AS "이메일_아이디"
FROM EMPLOYEE
ORDER BY "이메일_아이디";

-- INSTR(EMAIL,1) 을 작성하게 되면 나타나는 결과
SELECT EMP_NAME,SUBSTR(EMAIL,1) AS "이메일_아이디"
FROM EMPLOYEE
ORDER BY "이메일_아이디";
...
추후에 java int 값이 아니라 Sring 값으로 값이 넘어오고
추후 변환이 필요할 대는 TO_NUBER 사용할 일이 있을 수 있음
*/
-- DUAL 가상 테이블을 이용해서
