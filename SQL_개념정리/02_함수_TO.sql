...
-- 화폐기호 + 자릿수 구분
SELECT TO_CHAR(123456789,'L999999999'), -- L 9 아홉개
...
-- DUAL 테이블 사용해서 현재 날짜 시간(SYSDATE) 2024-06-05 (수) 오전 시:분:초
SELECT SYSDATE,TO_CHAR(SYSDATE,'YYYY-MM-DD (DY) AM HH:MI:SS')
FROM DUAL;

-- DUAL 테이블 사용해서 현재 날짜 시간(SYSDATE) 2024년06월05일 (수) 오전 00시 00분 00초
...
SELECT TO_DATE('05월27일 월요일 12시 50분','MM"월"DD"일" DAY HH"시"MI"분"')
FROM DUAL; 
-- TO_DATE 형식으로 년/월/일 변경하겠다
-- 05월27일 월요일 12시 50분 -> 24/05/27
