...
-- ANSI 방식 JOIN ON LACATION_ID = LOCAL_CODE
SELECT DEPT_TITLE AS "부서명",LOCAL_CODE AS "코드명",LOCAL_NAME AS "지역명"
FROM DEPARTMENT
JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE);

-- ORACLE 방식 LACATION_ID = LOCAL_CODE
SELECT DEPT_TITLE AS "부서명",LOCAL_CODE AS "코드명",LOCAL_NAME AS "지역명"
...
-- 2) 연결에 사용할 두 컬럼이 같은 경우 --> JOIN USING 을 사용
FROM 테이블명
...
WHERE DEPT_CODE(+) = DEPT_ID(+);
-- DEPT_CODE와 DEPT_ID 값이 일치하지 않아도
-- 양쪽에 있는 컬럼에 수에 맞게 표시를 넣어라 (NULL 값이라도 넣어라)
/*
