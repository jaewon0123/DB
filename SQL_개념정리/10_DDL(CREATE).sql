...
---- [ 자료형 ]
        ...
        VARCHAR2(크기) : 가변길이 문자형 (4000BYTE) VARCHAR1 의 업그레이드 버전
        
        ABC 문자열을 CHAR VARCHAR2 저장하면 어떻게 되는가?
        CHAR(10) 컬럼 : CHAR는 10을 모두 소진
        CARCHAR2(10) 컬럼 : VARCHAR2 3만큼만 소진 나머지 7은 반환
...
CREATE TABLE USER_MEMBER (USER_ID VARCHAR2(20),USER_PW VARCHAR2(20),USER_NAME VARCHAR(15),USER_SSN CHAR(14),ENROLL_DATE DATE DEFAULT SYSDATE);
...
-- 제약 조건
-- UPDATE PRIMARY KEY FOREIGN KEY CHECK 
/*
제약조건 (CON STR AIN TS)
사용자가 원하는 조건의 데이터만 유지하기 위해 특정컬럼에 설정하는 제약

데이터 무결성 보장을 목적으로 함
+ 입력 데이터에 문제가 없는지 자동으로 검사는 목적
...
KH_CAFE 테이블 (TABLE = OBJECT) 존재하기 때문에 만들 수 없음
*/
...
--> 변경 성공시 Table KH_CAFE이(가) 변경되었습니다.
COMMIT;
...
CREATE TABLE 테이블명 (
-- (방법 1)
컬럼명 바료형(정수자리값,실수자리값) PRIMARY KEY,
-- (방법 2)
컬럼명 바료형(정수자리값,실수자리값) CONSTRAINT 인덱스에저장할이름명 PRIMARY KEY,
-- (방법 3)
컬럼명 바료형(정수자리값,실수자리값)
...
INSERT INTO USER_PRIMARY_TABLE
VALUES (NULL,'이순신'); -- NULL 빈 값 넣는다는 의미
--> 기본키가 NULL 로 오류 발생
---> SQL 오류: ORA-01400: cannot insert NULL into
----> NULL 을 안에 삽입할 수 없습니다.

-- UNIQUE 와 PRIMARY의 차이점 --
-- UNIQUE 는 컬럼에서 NULL이 한개는 허용됨 두 개이상 안됨
...
-- USER_GRADE GRADE_CODE 참조하는 테이블을 생성
...
INSERT INTO USER_FK VALUES(4,'USER04','PW04',40);
--> 부모키인 USER_GRADE 에 40이라는 값이 존재하기 때문에 삽입 가능
...
VALUES (1,'USER01','QW01','홍길동','남');

INSERT INTO USER_CHECK
VALUES (2,'USER02','QW02','박철수','남자');
/*
ORA-02290: check constraint (KH_T.SYS_C007157) violated
제약조건으로 남이나 여만 가능하게 했는데 남자라는 조건이 들어와 에러 발생
*/
INSERT INTO USER_CHECK
VALUES (2,'USER02','QW02','박철수','여');

INSERT INTO USER_CHECK
VALUES (3,'USER03','QW03','강영희','여자');

INSERT INTO USER_CHECK
VALUES (3,'USER03','QW03','강영희','려');
...
-- 2번 방법 따로 INDEX 에 기록한다음 조건 설정 한 경우 (한 줄 작성)
컬럼명 자료형 CONSTRAINT 인덱스에기록할이름 CHECK(컬럼명 IN ('조건1','조건2'))

-- 3번 방법 따로 INDEX 에 기록한다음 조건 설정 한 경우 (여러 줄 작성)
컬럼명 자료형, 
