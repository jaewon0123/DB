...
-- 1. 이름의 세글자 가져오기
--> EMP_NAME AS 이름석자
SELECT EMP_NAME,  SUBSTR(EMP_NAME,1,3)AS "이름석자" FROM EMPLOYEE;

-- 2. 전화번호의 마지막 네자리 가져오기 -4
--> PHONE AS 마지막_네_자리
SELECT PHONE, SUBSTR(PHONE,-4) AS "마지막_네_자리" FROM EMPLOYEE;
--만약에 NULL 값이 아닌 번호만 보길 원한다면
...
-- 4. 전화번호의 앞 세자리 추출
--> PHONE AS 앞 세자리
SELECT PHONE, SUBSTR(PHONE,1,3) AS "앞_세자리" FROM EMPLOYEE;

-- 5. 이메일 @ 뒤에 있는 도메인 추출
SELECT EMP_NAME,SUBSTR(EMAIL,INSTR(EMAIL,'@')+1) AS "이메일_아이디"
FROM EMPLOYEE
ORDER BY "이메일_아이디";
...
--> D9 부서 직원 수
SELECT COUNT(DEPT_CODE) AS "D9_부서_직원수" FROM EMPLOYEE WHERE DEPT_CODE = 'D9';
-- 3. WHERE 활용해서 보너스 받은 직원 수
--> 보너스 받은 직원들
SELECT COUNT(BONUS) AS "보너스_받은_직원들" FROM EMPLOYEE WHERE BONUS IS NOT NULL;

/****** AVG 평균 구하기 실습 *******/
-- 1. 전체 직원 평균 급여 조회
--> AS 평균 급여
SELECT ROUND(AVG(SALARY)) AS "평균급여" FROM EMPLOYEE;
-- 2. 부서별 평균 급여
--> AS 부서, AS 평균급여
SELECT DEPT_CODE AS "부서", ROUND(AVG(SALARY)) AS "평균급여" FROM EMPLOYEE
GROUP BY DEPT_CODE;
/*
SELECT DEOT_CODE AS "부서", ROUND(AVG(SALARY)) AS "평균급여"
...
SELECT EMP_ID,JOB_CODE, REPLACE(JOB_CODE,'J','JOB') AS "수정된직무코드"
FROM EMPLOYEE;